    ...
    while not rospy.is_shutdown():
        ...
        if step == "get_bag" and sub_hand_cnt >= 1:
            ...
            if len(detection_list) <= 2 and len(detection_list) > 0:
                ...
                output_dir = "/home/pcms/catkin_ws/src/beginner_tutor ials/src/m1_evidence/"
        ...
        if action == "turn2":
            speed=0.2
            timems=target_distance//speed
            for i in range(timems):
                move(speed,0)
                time.sleep(0.01)
            q = [
                _imu.orientation.x,
        ...
        if action == "front":
            ...
            if t_idx != -1:
                ...
                cv2.circle(up_image, (target_cx, traget_cy), 5, (0, 255, 255), -1)
            ...
            if d<=100 and d!=-1 and d!=0:
                action = "grap"
                step = "none"
        if step == "check_voice":
            #thank you 西, ok 東, 90 北
            s = s.lower()
            print("speak", s)
            if "thank" in s or "you" in s or "ok" in s:
                if "thank" in s or "you" in s: dir_pos="east"
                elif "ok" in s: dir_pos="west"
                action = "none"
        ...
        if action == "follow":
            ...
            move(x, z)
            step = "check_voice"
        if action == "back_people1":
            left_right=[[],[],[],[],[]]
            right_left=[[],[],[],[],[]]
            main_list=[]
            if dir_pos == "left": main_list=left_right
            else: main_list=right_left
            change_mode=0
            for i in range(5):
                clear_costmaps
        ...
        if action == "back_people2":
            ...
            c_x,c_y,c_z=#get_chassis_place
            if d>=2500 and d!=0 and (#in position)L
                #always checking
                #arrive 指定
                action="back3"
        if action == "back3":
