...
if __name__ == "__main__":
    ...
    hl=0
    tgrx,tgry=0,0
    uuuuuuuuuuuuu=input("stepppppppppppppppppppppppppp:")
    if uuuuuuuuuuuuu == "ok":
        step="ft"
    while not rospy.is_shutdown():
        ...
        if step == "ft":
            ...
            #say("start")
            clear_costmaps
            chassis.move_to(2.602, -0.545, 0.470)
            # 門口
            # checking
            while not rospy.is_shutdown():
            ...
            #input("goooooooooo")
            clear_costmaps
            chassis.move_to(1.489, 0.796, 0.503)
            # 門口
            # checking
            while not rospy.is_shutdown():
                # 4. Get the chassis status.
            ...
            clear_costmaps
            chassis.move_to(2.783, -1.267,-1.078)
            # 原點
            while not rospy.is_shutdown():
                # 4. Get the chassis status.
                code = chassis.status_code
        ...
        if step == "_code":
            ...
            if step2 == "go":
                ...
                _, _, d = get_real_xyz(down_depth, tgrx,tgry,"down")
                timems = round(abs(d*0.5)) // 0.2 // 10 -15
                print(timems)
        ...
        if step == "grap":
            print("got there")
            open_gripper(t)
            joint1, joint2, joint3, joint4 = 0.0, 1.104, 0.758, -1.5
            ...
            if abs(e) <= 3:
                ...
                close_gripper(t)
                time.sleep(2)
                joint1, joint2, joint3, joint4 = 0.00, 1.104, 0.758, -1.5
                set_joints(joint1, joint2, joint3, joint4, 1)
                time.sleep(t)
                ...
                for i in range(132): 
                    move(0, -0.251)
                    time.sleep(0.1)
                say("Master please take this bag")
