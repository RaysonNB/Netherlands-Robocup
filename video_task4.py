    ...
    while not rospy.is_shutdown():
        ...
        if step == "follow":
            ...
            for i, pose in enumerate(poses):
                ...
                cx = (p5[0] + p6[0]) // 2
                cy = (p5[1] + p6[1]) // 2
                cv2.circle(down_image, p5, 5, (0, 0, 255), -1)
                cv2.circle(down_image, p6, 5, (0, 0, 255), -1)
            ...
            if t_idx != -1:
                ...
                cy = (p5[1] + p6[1]) // 2
                _, _, d = get_real_xyz(down_depth, cx, cy, 2)
                cv2.circle(down_image, (cx, cy), 5, (0, 255, 255), -1)

                print("people_d", d)
                if d >= 1800 or d == 0: continue

                x, z, down_image, yn = _fw.calc_cmd_vel(down_image, down_depth, cx, cy)
                print("turn_x_z:", x, z)
            move(x, z)
            action = "check_voice"
